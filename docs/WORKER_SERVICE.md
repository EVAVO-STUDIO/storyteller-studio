...
The worker service is the long-running internal process that joins the durable queue, immutable generation material, provider adapters, lease heartbeat, private object storage and artifact registry.
...
Each active task owns its own queue lease and heartbeat controller. Completion of one task permits another claim without increasing the maximum number of simultaneous provider executions.
...
A valid claim and material record enter `runGenerationWorkerWithHeartbeat` with:
...
The service does not add another generation implementation. It composes the previously verified provider, storage, artifact and heartbeat boundaries.

## Live transition time

Provider execution may outlive one or more heartbeat renewals. Artifact writes and terminal queue transitions therefore use the live service clock rather than the timestamp captured when the claim first began.
...
The service retains a bounded outcome history containing queue and job identifiers, counts, safe finding codes, revision and redacted worker result. It does not retain manuscript text, voice identities, provider credentials, private request identifiers, object locators or lease secrets.

Unexpected internal failures are converted to a retryable queue failure only while the original claim remains authoritative. When the current queue state no longer belongs to this worker, the service records ownership loss and performs no terminal write.
...
4. stop each heartbeat before its terminal transition;
...
Interrupted claims are not falsely completed, blocked or failed by a stale shutdown path. They remain leased until expiry and are later recovered by the queue reaper.
...
Ordinary expected outcomes such as missing material, provider configuration blocks, retryable provider failure, artifact quarantine, cost policy failure, ownership loss and operator abort do not crash the service.
...
A deployment wrapper will be a dedicated non-HTTP worker process with a restricted service identity.
...
- PostgreSQL transactional claims;
