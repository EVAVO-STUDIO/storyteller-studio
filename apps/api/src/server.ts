...
import type { FileCalibrationSessionStore } from "@evavo/storyteller-engine/calibration-store";
import {
...
import {
  ...
  type CalibrationStoreRuntimeConfiguration,
...
function configuredMaximumRequestBytes(environment: NodeJS.ProcessEnv): number {
  const parsed = Number.parseInt(
    environment.STORYTELLER_MAX_REQUEST_BYTES ?? "",
    10,
  );
...
function configuredActorId(environment: NodeJS.ProcessEnv): string {
  ...
  if (!actorId) {
    if (isProduction(environment)) {
      throw new Error("API_ACTOR_CONFIGURATION_MISSING");
    }
    return "local_operator";
  }
  if (!ACTOR_ID_PATTERN.test(actorId)) {
    throw new Error("API_ACTOR_CONFIGURATION_INVALID");
  }
  return actorId;
}
...
function authorise(
  ...
  if (!expected) {
    if (isProduction(environment)) {
      return {
        ok: false,
        status: 503,
        code: "API_AUTH_CONFIGURATION_MISSING",
      };
    }
    return isLoopback(request)
      ...
      : {
          ok: false,
          status: 403,
          code: "API_DEVELOPMENT_LOOPBACK_ONLY",
        };
  }
  ...
  if (!authorization?.startsWith("Bearer ")) {
    return {
      ok: false,
      status: 401,
      code: "API_BEARER_TOKEN_REQUIRED",
    };
  }
...
async function readJson(
  ...
  for await (const chunk of request) {
    ...
    if (received > maximumBytes) {
      throw new Error("API_REQUEST_BODY_TOO_LARGE");
    }
    chunks.push(buffer);
  }
  if (chunks.length === 0) return null;
  const contentType = request.headers["content-type"] ?? "";
  if (!contentType.toLocaleLowerCase("en-AU").includes("application/json")) {
    throw new Error("API_JSON_CONTENT_TYPE_REQUIRED");
  }
  try {
...
function parseProjectInput(value: unknown): CreateProjectInput {
  ...
  if (
    typeof value.manuscriptText !== "string"
    || value.manuscriptText.trim().length === 0
  ) {
    throw new Error("PROJECT_MANUSCRIPT_INVALID");
  }
  if (!isRecord(value.rightsEvidence)) {
    throw new Error("PROJECT_RIGHTS_EVIDENCE_INVALID");
  }
  if (!isRecord(value.providerRequirements)) {
    throw new Error("PROJECT_PROVIDER_REQUIREMENTS_INVALID");
  }
  if (!Array.isArray(value.providerProfiles)) {
    throw new Error("PROJECT_PROVIDER_PROFILES_INVALID");
  }
  return value as unknown as CreateProjectInput;
}

function parseProviderRequest(value: unknown): {
  ...
  if (
    ...
    throw new Error("PROVIDER_RANKING_INPUT_INVALID");
  }
...
function parseTakeObservation(value: unknown): TakeObservation {
  ...
  if (!isRecord(value.audio) || !isRecord(value.deliveryProfile)) {
    throw new Error("TAKE_AUDIO_OBSERVATION_INVALID");
  }
...
function parseManifest(value: unknown): ProjectManifest {
  if (
    !isRecord(value)
    || value.schemaVersion !== "storyteller-project-v1"
    || typeof value.id !== "string"
    || !isRecord(value.manuscript)
  ) {
...
function parseOptionalDate(value: unknown): Date | undefined {
  ...
  if (typeof value !== "string") {
    throw new Error("GENERATION_QUEUE_AVAILABLE_AT_INVALID");
  }
...
function parseQueueStatus(value: string): GenerationQueueStatus {
  if (!QUEUE_STATUS_VALUES.includes(value as GenerationQueueStatus)) {
    throw new Error("GENERATION_QUEUE_STATUS_FILTER_INVALID");
  }
  return value as GenerationQueueStatus;
}

function parseQueueStatuses(
  url: URL,
): readonly GenerationQueueStatus[] | undefined {
...
function parseQueueProjectId(url: URL): string | undefined {
  ...
  if (!QUEUE_ITEM_ID_PATTERN.test(value)) {
    throw new Error("GENERATION_QUEUE_PROJECT_FILTER_INVALID");
  }
  return value;
}
...
function parseQueueLimit(url: URL): number {
  ...
  return parseInteger(
    ...
    1,
    200,
    "GENERATION_QUEUE_LIMIT_INVALID",
  );
}
...
function decodeQueueItemId(value: string): string {
  ...
  if (!QUEUE_ITEM_ID_PATTERN.test(decoded)) {
    throw new Error("GENERATION_QUEUE_ID_INVALID");
  }
  return decoded;
}

function queueItemRoute(pathname: string): Readonly<{
  itemId: string;
  action: "read" | "cancel";
}> | null {
  const cancel = pathname.match(
    /^\/v1\/generation\/queue\/([^/]+)\/cancel$/u,
  );
  if (cancel?.[1]) {
    return {
      itemId: decodeQueueItemId(cancel[1]),
      action: "cancel",
    };
  }
  ...
  if (read?.[1]) {
    return {
      itemId: decodeQueueItemId(read[1]),
      action: "read",
    };
  }
  return null;
...
function errorStatus(code: string): number {
  ...
  if (
    ...
    || code === "CALIBRATION_STORE_NOT_CONFIGURED"
    || code === "CALIBRATION_STORE_DATA_DIR_REQUIRED"
    || code === "CALIBRATION_STORE_DRIVER_INVALID"
    || code === "CALIBRATION_STORE_FILE_DRIVER_SINGLE_HOST_ACK_REQUIRED"
...
export function createStorytellerApiHandler(
  options: StorytellerApiHandlerOptions = {},
) {
  ...
  let calibrationConfiguration: CalibrationStoreRuntimeConfiguration | undefined;
  let calibrationRuntime: FileCalibrationSessionStore | null | undefined;

  const resolveQueueConfiguration = (): GenerationQueueRuntimeConfiguration => {
    queueConfiguration ??= resolveGenerationQueueRuntimeConfiguration(
      environment,
      workingDirectory,
    );
  ...
  const requireQueueRuntime = (): FileGenerationQueue => {
    if (queueRuntime === undefined) {
      queueRuntime = createGenerationQueueRuntime(resolveQueueConfiguration());
    }
  ...
  const queueHealth = (): Readonly<Record<string, unknown>> => {
    try {
      const summary = generationQueueRuntimeSummary(
        resolveQueueConfiguration(),
      );
      return {
        status: summary.enabled ? "ready" : "disabled",
        ...summary,
      };
    } catch (error) {
      const code = error instanceof Error
        ? error.message
        : "GENERATION_QUEUE_CONFIGURATION_INVALID";
      return {
  ...
  const resolveArtifactConfiguration = (): ArtifactRegistryRuntimeConfiguration => {
    artifactConfiguration ??= resolveArtifactRegistryRuntimeConfiguration(
      environment,
      workingDirectory,
    );
  ...
  const requireArtifactRuntime = (): FileArtifactRegistry => {
    if (artifactRuntime === undefined) {
      artifactRuntime = createArtifactRegistryRuntime(
        resolveArtifactConfiguration(),
      );
    }
  ...
  const artifactHealth = (): Readonly<Record<string, unknown>> => {
    try {
      const summary = artifactRegistryRuntimeSummary(
        resolveArtifactConfiguration(),
      );
      return {
        status: summary.enabled ? "ready" : "disabled",
      ...
      const code = error instanceof Error
        ? error.message
        : "ARTIFACT_REGISTRY_CONFIGURATION_INVALID";
      return {
        status: "misconfigured",
        enabled: false,
  ...
  const resolveCalibrationConfiguration = (): CalibrationStoreRuntimeConfiguration => {
    calibrationConfiguration ??= resolveCalibrationStoreRuntimeConfiguration(
      environment,
      workingDirectory,
    );
  ...
  const requireCalibrationRuntime = (): FileCalibrationSessionStore => {
    if (calibrationRuntime === undefined) {
      calibrationRuntime = createCalibrationStoreRuntime(
        resolveCalibrationConfiguration(),
      );
    }
    if (!calibrationRuntime) {
      throw new Error("CALIBRATION_STORE_NOT_CONFIGURED");
    }
    return calibrationRuntime;
  };

  const calibrationHealth = (): Readonly<Record<string, unknown>> => {
    try {
      const summary = calibrationStoreRuntimeSummary(
        resolveCalibrationConfiguration(),
      );
      return {
        status: summary.enabled ? "ready" : "disabled",
      ...
      const code = error instanceof Error
        ? error.message
        : "CALIBRATION_STORE_CONFIGURATION_INVALID";
      return {
        status: "misconfigured",
        enabled: false,
  ...
  return async function handler(
    ...
    try {
      if (request.method === "GET" && url.pathname === "/health") {
        ...
        const calibrationStore = calibrationHealth();
        status = [generationQueue, artifactRegistry, calibrationStore].some(
          (component) => component.status === "misconfigured",
        )
          ? 503
          : 200;
      ...
      if (request.method === "GET" && url.pathname === "/v1/capabilities") {
        ...
        sendJson(response, id, status, {
          ...
          performance: [
            ...
            "series continuity assessment",
          ],
          quality: [
            "transcript fidelity",
            "audio delivery profiles",
            "candidate take selection",
            "sustained-listening calibration",
          ],
          governance: [
          ...
          calibration: [
            "redacted session reads",
            "reviewer aggregation",
            "dimension score summaries",
            "provider continuity locks",
          ],
          visuals: [
            "scene-level beat grouping",
            "continuity keys",
            "restrained motion policies",
          ],
          workerApiExposed: false,
      ...
      if (url.pathname.startsWith("/v1/calibrations")) {
        const calibrationRoute = await handleCalibrationReadRoute({
          method: request.method,
          url,
          store: request.method === "GET"
            ? requireCalibrationRuntime()
            : null,
          requestId: id,
        });
        if (calibrationRoute) {
          status = calibrationRoute.status;
          sendJson(response, id, status, calibrationRoute.body);
          return;
        }
      }
      ...
      if (request.method === "POST" && url.pathname === "/v1/providers/rank") {
        ...
        status = 200;
        sendJson(response, id, status, {
          data: rankProviders(input.requirements, input.profiles),
          requestId: id,
        });
        return;
      }
      ...
      if (request.method === "POST" && url.pathname === "/v1/takes/evaluate") {
        const observation = parseTakeObservation(
          await readJson(request, environment),
        );
      ...
      if (request.method === "POST" && url.pathname === "/v1/generation/jobs") {
        ...
        if (!isRecord(payload)) {
          throw new Error("GENERATION_JOB_INPUT_INVALID");
        }
      ...
      if (request.method === "POST" && url.pathname === "/v1/generation/queue") {
        const payload = await readJson(request, environment);
        if (!isRecord(payload)) {
          throw new Error("GENERATION_QUEUE_INPUT_INVALID");
        }
        const manifest = parseManifest(payload.manifest);
        const candidateCount = parseInteger(
          payload.candidateCount,
        ...
        const hasRunnable = data.some((item) =>
          item.status === "queued"
      ...
      if (queueRoute?.action === "read" && request.method === "GET") {
        ...
        if (!envelope) {
          throw new GenerationQueueConflictError(
            "GENERATION_QUEUE_ITEM_NOT_FOUND",
          );
        }
        status = 200;
        sendJson(response, id, status, {
      ...
      if (queueRoute?.action === "cancel" && request.method === "POST") {
        ...
        if (!isRecord(payload) || typeof payload.reason !== "string") {
          throw new Error("GENERATION_QUEUE_CANCELLATION_INPUT_INVALID");
        }
        const envelope = await requireQueueRuntime().cancel(
          queueRoute.itemId,
          {
...
export function startStorytellerApi(): void {
  ...
  const port = Number.parseInt(
    process.env.STORYTELLER_API_PORT ?? "3100",
    10,
  );
